var YZM = {
	...
	'player':{
		'play':function(url){
			var type = url.indexOf('.m3u8') > -1 ? 'customHls' : 'auto';
			...
			YZM.dp=new yzmplayer({
				...
				theme:config.color,
				logo:config.logo,
				video:{url:url,pic:config.pic,type:type,
  				customType: {
		...
		'dmplay':function(url){
			...
			YZM.dp=new yzmplayer({
				...
				theme:config.color,
				logo:config.logo,
				contextmenu :[{text:YZM.link.split(",")[0],link:YZM.link.split(",")[1]}],
				video:{url:url,pic:config.pic,type:type,
  				customType: {
       				           'customHls': function (video, player) {
         				             const hls = new Hls({
